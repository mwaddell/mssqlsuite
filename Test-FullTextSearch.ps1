...
$Status = sqlcmd -S localhost -U $UserName -P $Password -Q "SELECT servicename, status_desc FROM sys.dm_server_services WHERE servicename LIKE 'SQL Full-text Filter Daemon Launcher%';" | Select-String -Pattern "Running" -Quiet
